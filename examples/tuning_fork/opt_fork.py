...
if __name__ == "__main__":
    ...
    def loc_dv_list(length, thick=5e-3, Mmass=0.1, mx=0.5):
        # starting dv list
        return ([length] + [thick]*4 + [Mmass, mx])
    ...
    # base and level 1
    init_thick = 4e-2 # 3e-2
    init_design = loc_dv_list(0.3, init_thick) * 9
    tree_start_nodes = [0] + [1]*4 + [2,3,4,5]
    tree_directions = [5] + [0,1,2,3] + [5]*4

    # reload init_design from prev opt design of level 1
    # --------------------------------------------------

    init_design = [0.40774859, 0.08255792, 0.03415702, 0.06825633, 0.04095295,
    ...
    # start out very small (it can add it in as it goes)
    L2 = 1e-2 # 0.1
    ...
    if args.level == 2:
        ...
        # level 2, x- branch
        init_design += loc_dv_list(L2, Mmass=1e-2) * 8
